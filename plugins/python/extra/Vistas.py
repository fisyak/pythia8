...
class Vistas:
    ...
    """
    #----------------------------------------------------------------------
    def __init__(self, pythia, selector =
                 lambda event, idx: len(event[idx].daughterList()) != 1
        ...
        for key, val in self.cdb.items():
            if key in skip:
                continue
            val["color"] = Vistas.color(
                1.5*idx/(len(self.cdb) - len(skip) - 1) - 1)
            idx += 1
    ...
    def data(self, event = None, jets = None):
        ...
        if self.opts["frame"] in ["hard process", "beam"]:
            ...
            bst = sum((prt.p() for prt in self.bst if prt.statusAbs() == code),
                      Vec4(0, 0, 0, 0))
            bst.flip3()
            self.bst.bst(bst)
        ...
        if (event is None and self.pythia.infoPython().nMPI() != (
            ...
            self.log(1, f"{len(self.hard) + len(self.mpis)} of " +
                     f"{self.pythia.infoPython().nMPI()} scatters found")

        # Determine the min/max observables per category (if needed).
        obs = [self.opts[opt]["observable"] for opt in ("length", "color")
               if self.opts[opt]["observable"]]
        if len(obs) > 0:
            self.ocats = {cat: {o: [float("inf"), float("-inf")] for o in obs}
        ...
        for idx, mpi in enumerate(self.hard + self.mpis):
            self.saveVertex(dct = data["Pythia 8 Event"]["Tracks"],
                            vrt = mpi, pos = pos)
            pos = Vec4(stp)
        ...
        for vrt in self.part:
            self.saveVertex(dct = data["Pythia 8 Event"]["Tracks"],
                            vrt = vrt, pos = vrt.pos, reverse = False)

        # Save the color flow to JSON.
        self.saveColorFlow(
            dct = data["Pythia 8 Event"]["Tracks"]["color flow"])

        # Save the jets.
        if "Jets" in data["Pythia 8 Event"]:
            self.saveJets(
                dct = data["Pythia 8 Event"]["Jets"])
    ...
    def config(self):
        ...
        level = lambda name, node, toggle, children: {
            "name": name, "nodeLevel": node, "toggleState": toggle,
            "childrenActive": children, "configs": [], "children": []}
        ...
        for key in self.cdb:
            ...
            for obs, obsMin, obsMax, obsStep in (
                    ("\u03d5", -3.14, 3.14, 0.01), ("\u03b7", -4, 4, 0.1),
                    ("pT", 0, 50000, 0.1)):
                cuts["configs"].append({
                    "type": "rangeSlider", "label": obs, "min": obsMin,
                    "max": obsMax, "step": obsStep, "value": obsMin,
                    "highValue": obsMax, "enableMin": False,
                    "enableMax": False})
    ...
    def options(self):
        ...
        opts = {
            ...
            "highlight": [],
            "mpi": 1,
            "jets": {"algorithm": None, "r": 0.5, "ptmin": 20, "etamax": 25,
                     "select": "all", "mass": "gen", "observable": "p.e()"},
            "length": {"scale": "constant", "observable": "p.e()",
                       "skew": 1e5, "group": "cat", "factor": 80, "offset": 1},
            "color": {"scale": "log", "observable": "p.e()",
                      "skew": 10, "min": -0.4, "max": 0.4, "group": "cat"},
    ...
    #----------------------------------------------------------------------
    def display(
            self, data = True, cfg = True, sleep = 5, pad = 0.5,
            upload = "FzAX60WBp5hZbZX", download = "689N2msjNEXY6Dr",
        ...
        for path in (data, cfg):
            ...
            request = urllib.request.Request(
                f"{cernbox}/{upload}/{path}", method = "DELETE",
    ...
    def upload(data, upload, cernbox):
        ...
        # Create the request.
        request = urllib.request.Request(
            f"{cernbox}/{upload}/{name}", data = data, method = "PUT",
            headers = {"X-Requested-With": "XMLHttpRequest"})
        # Upload.
        try:
        ...
        if not status in [201, 204]:
            raise RuntimeError(
                f"CERNBOX rejected the upload with status {status}"
            )
        return name
    ...
    def addVertex(self, idx, dtr = None):
        ...
        # Create the vertex otherwise.
            ...
            moms = [
                mom for mom in (self.addParticle(i) for i in idxs) if mom.use]
    ...
    def addParticle(self, idx):
        ...
        # Create the particle otherwise.
            ...
            prt = self.Particle(
                key = key, pro = None, end = None, prt = self.bst[idx], use =
                self.selector is None or self.selector(self.bst, idx),
                cdb = self.cdb)
            self.prts[key] = prt
        # Return the particle.
        return prt
    ...
    def saveVertex(self, dct, vrt, pos, reverse = True):
        ...
        if reverse:
            for mom in vrt.moms:
                self.saveParticle(
                    dct = dct, prt = mom, pos = pos, reverse = True)
    ...
    def saveParticle(self, dct, prt, pos, reverse = False):
        ...
        nrm = att["factor"]*self.strength(
            self.lab[prt.key], att, prt.status.name) + att["offset"]
        stp = prt.prt.p()
        if stp.pAbs() == 0:
        ...
        # Otherwise, set the line color.
            ...
            att = self.opts["color"]
            dif = att["max"] - att["min"]
            nrm = dif*self.strength(
                self.lab[prt.key], att, prt.status.name) + att["min"]
        ...
        if cc != 0 or ac != 0:
            ...
            if prt.end is None or sum(
                    abs(dtr.prt.colType()) for dtr in prt.end.dtrs) == 0:
                if cc != 0:
    ...
    def saveJets(self, dct):
        ...
        for key, jets in self.jets.items():
            ...
            for pl, pb in jets:
                nrm = att["factor"]*(
                    self.strength(pl, att, key) + att["offset"])
                dct[key].append(
                    {
                        "px": pl.px(),
                        "py": pl.py(),
    ...
    def statuses(path):
        ...
        clean = lambda s: re.sub(r"\s+", " ", html.unescape(
            re.sub(r"<[^>]+>", "", s))).strip()
        ...
        with open(path, encoding="utf-8") as xml:
            raw = xml.read()
        block = re.search(
            r'<method name="int Particle::status\(\)">(.*?)</method>',
